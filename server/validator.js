...
function statusData(data) {
  ...
  // Vérifier le format de stream_start_time s'il est présent
  if ('stream_start_time' in data && data.stream_start_time !== null) {
    // S'assurer que c'est une chaîne formatée comme une date ISO
    if (typeof data.stream_start_time !== 'string' || isNaN(Date.parse(data.stream_start_time))) {
      return false;
    }
  }
